...
const VideoRecorder = () => {
  ...
  const [selectedTest, setSelectedTest] = useState('Sit Ups');
  ...
  const uploadVideo = async () => {
    ...
    try {
      ...
      const analysisRes = await authAxios.post('/api/performance/analyze', { videoUrl, testType: selectedTest });
      ...
      alert('Failed to upload video or start analysis: ' + (error.response ? error.response.data.msg : error.message));
    } finally {
      setIsUploading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100">
      <h1 className="text-4xl font-bold text-gray-800 mb-8">Video Assessment</h1>
      <div className="my-4">
        <label htmlFor="test-select" className="mr-2 font-semibold">Select a Test:</label>
        <select 
          id="test-select" 
          value={selectedTest} 
